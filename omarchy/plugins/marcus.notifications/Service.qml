...
Item {
  ...
  function invokePopupDefault(index) {
    ...
    // Focus first so a chat client's default action cannot race the focus
    // request by closing or replacing the notification.
    focusApp(entry)
  ...
  // Try both the display name and desktop-entry icon. Chat clients commonly
  // report a human-readable app name (for example, "Telegram Desktop") while
  // Hyprland exposes the desktop-entry id as the window class.
  function focusApp(entry) {
    if (!entry || (!entry.app && !entry.appIcon)) return
    focusAppProc.command = [
      "bash",
      service.home + "/.config/omarchy/bin/focus-notification-app",
      service.omarchyPath,
      String(entry.app || ""),
